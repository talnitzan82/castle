...
$impDetails['cost'] = (isset($_GET['cost'])  && is_numeric($_GET['cost'])) ? $_GET['cost'] : 0;
$impDetails['bid'] = (isset($_GET['bid']) && is_numeric($_GET['bid'])) ? $_GET['bid'] : 0;
$impDetails['camp_id'] = (isset($_GET['camp_id']) && is_numeric($_GET['camp_id'])) ? $_GET['camp_id'] : 0;
$impDetails['creative_id'] = (isset($_GET['creative']) && is_numeric($_GET['creative'])) ? $_GET['creative'] : 0;
$impDetails['type'] = isset($_GET['c']) ? $_GET['c'] : "c";
$impDetails['s_system'] = isset($_GET['s_system']) ? $_GET['s_system'] : "no_system";
$impDetails['height'] = $_GET['height'];
$impDetails['width'] = $_GET['width'];
$height = is_numeric($impDetails['height']) ? str_replace("'","",$_GET['height']) : 250;
$width = is_numeric($impDetails['width']) ? str_replace("'","",$_GET['width']) : 300;

$impDetails['referer'] = extract_domain(processURLString(getReferer()));
$top_domain = $impDetails['top_domain'] = isset($_GET['domain']) && strlen($_GET['domain'])>0 ? extract_domain(processURLString(urldecode($_GET['domain']))) : $impDetails['referer'];
...
// check for the media trust
if (strpos($impDetails['referer'], 'mediatrust') !== false) {
    $isAudit = true;
}
...
function insertImpression($impDetails) {
	...
	$arrInsert[] = "creative_id='".mysqli_real_escape_string($db_con, $impDetails['creative_id'])."'";
